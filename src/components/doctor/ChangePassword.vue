									...
									<div class="card-body">
										<div class="row">
											<div class="col-md-12 col-lg-6">
												...
												<form @submit="changeTherapistPassword">
													
													<div class="form-group">
														<label>New Password</label>
														<input v-model="therapistPassword" type="password" class="form-control">
													</div>
													<div class="form-group">
														<label>Confirm Password</label>
														<label class="text-danger" v-show="showDangerPass">Password Not matched</label>
														<input v-on:blur="checkPass" v-model="therapistPasswordAgain" type="password" class="form-control">
													</div>
...
export default {
    ...
    methods:{
        checkPass(){
            if(this.therapistPassword === this.therapistPasswordAgain)
             {
        ...
        changeTherapistPassword(){
           ...
           let token = window.localStorage.getItem('token');
            ...
            TherapistService.updateTherapistPasswordService(token, this.therapistPassword).then((response) => {
