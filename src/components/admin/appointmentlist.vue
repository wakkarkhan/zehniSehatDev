									...
									<table class="datatable table table-hover table-center mb-0">
										...
										<tbody>
											<tr v-for="item in appointment" :key="item.id">
												...
												<td>{{item.therapist_speciality}}</td>
												<td>
													<h2 class="table-avatar">
														<router-link to="/admin/profile" class="avatar avatar-sm mr-2"><img class="avatar-img rounded-circle" :src="loadImg()" alt="User Image"></router-link>
												...
												<td>{{item.created_at}} <span class="text-primary d-block">Time</span></td>
												<td>
...
import AdminService from '@/api-services/admin.service';
	...
	methods:{
		getAllAppointments(){
            ...
            AdminService.AllAppointmentsService(token).then((response) => {
        ...
        loadImg() {
			...
			// } else {
			 	return doctorsimages('./' + 'doctor-01.jpg')
			// }
        },
    },
    mounted() {
